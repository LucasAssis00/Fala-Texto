...
class Preencher(App):
    ...
    def show_warning2(self, transcricao_inicial):
        layout = BoxLayout(orientation='vertical', spacing=10, padding=10)

        self.campo_texto_popup = TextInput(
            text=transcricao_inicial,
            multiline=True,
            font_size=18,
            size_hint=(1, 0.8)
        )
        ...
        botao_confirmar.bind(on_press=self.ao_confirmar_transcricao)

        layout.add_widget(self.campo_texto_popup)
        layout.add_widget(botao_confirmar)

        self.popup_edicao.open()

    def ao_confirmar_transcricao(self, instance):
        texto_final = self.campo_texto_popup.text
        self.popup_edicao.dismiss()

        # Ação com o texto final (você pode chamar outro método aqui)
        print("Texto final confirmado:", texto_final)
