...
def reconhecer_comando(interrupcao):
    #global interrupcao
    recognizer = sr.Recognizer()
    ...
    with sr.Microphone() as source:
        ...
        while True:
            ...
            try:
                ...
                if "ZELDA INTERROMPER" in stringEntrada:
                    if interrupcao == False:
                        speak("Sistema em modo de espera")
                    interrupcao = True
                #elif "ZELDA INICIAR" in stringEntrada or "ZELDA CONTINUAR" in stringEntrada:
                elif "ZELDA" in stringEntrada:
                    if interrupcao == True:
                        speak("Olá. Fale os campos e as informações a serem escritas neles.")
                    interrupcao = False
...
def iniciar_reconhecimento(interrupcao):
    thread = threading.Thread(target=reconhecer_comando(interrupcao))
    thread.daemon = True
    thread.start()
...
iniciar_reconhecimento(interrupcao)
